...
function setup_required_packages
{
    ...
    apt-get -y install --fix-missing \
        build-essential${VERSIONS[build-essential]} \
        autoconf${VERSIONS[autoconf]} \
        flex${VERSIONS[flex]} \
        nfs-kernel-server${VERSIONS[nfs-kernel-server]} \
        corosync${VERSIONS[corosync]} \
        pacemaker${VERSIONS[pacemaker]} \
        resource-agents${VERSIONS[resource-agents]} \
        systemd${VERSIONS[systemd]} \
        crmsh${VERSIONS[crmsh]}
