...
function get_setup_params_from_configs_json
{
    ...
    export useVarnish=$(echo $json | jq -r .moodleProfile.useVarnish)
    export createMoodleDatabase=$(echo $json | jq -r .moodleProfile.createMoodleDatabase)
    export installMoodle=$(echo $json | jq -r .moodleProfile.installMoodle)
    export setCronnedSQLDump=$(echo $json | jq -r .moodleProfile.setCronnedSQLDump)
    export useXSendFile=$(echo $json | jq -r .moodleProfile.useXSendFile)
    export modifyMoodleConfigFiles=$(echo $json | jq -r .moodleProfile.modifyMoodleConfigFiles)
    export useNginxAsReverseProxy=$(echo $json | jq -r .moodleProfile.useNginxAsReverseProxy)
    export additionalMSSQL=$(echo $json | jq -r .moodleProfile.additionalMSSQL)
    export localeToGenerate=$(echo $json | jq -r .moodleProfile.localeToGenerate)
}
...
function install_php_mssql_driver
{
    ...
    sudo apt-get -y install gnupg${VERSIONS[gnupg]}
    ...
    sudo ACCEPT_EULA=Y apt-get install -y \
        msodbcsql17${VERSIONS[msodbcsql17]} \
        mssql-tools${VERSIONS[mssql-tools]} \
        unixodbc-dev${VERSIONS[unixodbc-dev]}
    echo 'export PATH="$PATH:/opt/mssql-tools/bin"' >> ~/.bash_profile
    echo 'export PATH="$PATH:/opt/mssql-tools/bin"' >> ~/.bashrc
    source ~/.bashrc

    #Build mssql driver
    /usr/bin/pear config-set php_ini /etc/php/$PHPVER/fpm/php.ini system
    /usr/bin/pear config-set php_ini /etc/php/$PHPVER/cli/php.ini system
    /usr/bin/pecl install sqlsrv${VERSIONS[sqlsrv]}
    /usr/bin/pecl install pdo_sqlsrv${VERSIONS[pdo_sqlsrv]}
    PHPVER=$(get_php_version)
    echo extension=sqlsrv.so >> /etc/php/$PHPVER/fpm/conf.d/20-sqlsrv.ini
    echo extension=pdo_sqlsrv.so >> /etc/php/$PHPVER/fpm/conf.d/30-pdo_sqlsrv.ini
    echo extension=sqlsrv.so >> /etc/php/$PHPVER/cli/conf.d/20-sqlsrv.ini
    echo extension=pdo_sqlsrv.so >> /etc/php/$PHPVER/cli/conf.d/30-pdo_sqlsrv.ini
}
...
LOCAL_TIMESTAMP_FULLPATH="/var/www/html/.last_modified_time.moodle_on_azure"
...
if [ -f "$SERVER_TIMESTAMP_FULLPATH" ]; then
  ...
  if [ "\$SERVER_TIMESTAMP" != "\$LOCAL_TIMESTAMP" ]; then
    ...
    cp /moodle/html/config-moodle.php /var/www/config-moodle.php >> $SYNC_LOG_FULLPATH
    cp /moodle/moodledata/muc/config.php /moodlelocal/muc/config.php >> $SYNC_LOG_FULLPATH
  fi
else
